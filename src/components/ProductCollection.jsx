...
const visibleProducts=filteredProducts.slice(scrollIndex,scrollIndex+itemsPerView);
...
setScrollIndex(Math.min(scrollIndex+itemsPerView,filteredProducts.length-itemsPerView));
...
Our Products <span className="text-[#c97979]">Collections</span>
</h2>
...
key={tab.key}
to="/categories"
className="px-6 py-2 rounded-full border border-[#ead7cf] bg-[#fff7f4] hover:bg-[#f3e3dc]"
>
{tab.label}
</Link>
...
<button
key={tab.key}
onClick={()=>{setActiveTab(tab.key);setScrollIndex(0);}}
className={`px-6 py-2 rounded-full border border-[#ead7cf] ${
activeTab===tab.key
? "bg-[#d98b8b] text-white"
: "bg-[#fff7f4]"
}`}
>
{tab.label}
...
<button
onClick={scrollLeft}
className="absolute left-0 top-1/2 -translate-y-1/2 bg-[#fff7f4] shadow rounded-full p-2 z-10"
>
<ChevronLeft size={20}/>
</button>
...
<button
onClick={scrollRight}
className="absolute right-0 top-1/2 -translate-y-1/2 bg-[#fff7f4] shadow rounded-full p-2 z-10"
>
